...
let migrate5 (conn: IDbConnection) =
    task {
        let! results =
            select {
                for m in MigrationTable do
        ...
        if results.Count() = 0 then
            let! _ =
                """
                create index play_amount_stats_game_id_month
                on PlayAmountStats (GameId, Month)
                """
                |> conn.ExecuteAsync
...
let migrate9 (conn: IDbConnection) =
    task {
        let! results =
            select {
                for m in MigrationTable do
        ...
        if results.Count() = 0 then
            let! _ =
                """
                create index player_count_vote_by_game
                on PlayerCountVote (GameId)
                """
                |> conn.ExecuteAsync
